...
# === LASSO ===
lasso = LassoCV(alphas=np.logspace(-4,4,50), max_iter=5000)
...
# === Elastic Net ===
enet = ElasticNetCV(l1_ratio=[0.2,0.5,0.8], alphas=np.logspace(-4,4,50), max_iter=5000)
...
# === Random Forest ===
rf = RandomForestRegressor(n_estimators=200, max_depth=6, max_features='sqrt', random_state=42, n_jobs=-1)
...
# === Gradient Boosting ===
gbrt = GradientBoostingRegressor(n_estimators=200, max_depth=3, learning_rate=0.05)
...
pred_df.to_csv('../data/vol-output/ml_base_predictions_v.csv', index=False)
print("✅ Saved ../data/vol-output/ml_base_predictions_v.csv")

# === Save metrics ===
metrics_df = pd.DataFrame(results).T
metrics_df.to_csv('../data/vol-output/ml_model_metrics_v.csv')
print("✅ Saved ../data/vol-output/ml_model_metrics_v.csv")

r2_df = metrics_df[['R2']].rename(columns={'R2': 'Test_R2'})
r2_df.to_csv('../data/vol-output/ml_r2_v.csv')
print("✅ Saved ../data/vol-output/ml_r2_v.csv")
