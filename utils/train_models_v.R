...
# === Load dataset ===
df <- read_delim("../data/crsp_features_full_v.txt", delim = "\t")
...
# === Ridge ===
ridge <- cv.glmnet(X_train, y_train, alpha = 0, lambda = 10^seq(-4, 4, length.out = 50))
...
# === LASSO ===
lasso <- cv.glmnet(X_train, y_train, alpha = 1, lambda = 10^seq(-4, 4, length.out = 50))
...
# === Elastic Net ===
enet <- cv.glmnet(X_train, y_train, alpha = 0.5, lambda = 10^seq(-4, 4, length.out = 50))
...
# === Gradient Boosting ===
gbrt <- gbm(y_train ~ ., data = as.data.frame(X_train),
            n.trees = 200, interaction.depth = 3,
            shrinkage = 0.05, distribution = "gaussian", verbose = FALSE)
...
write_csv(pred_df, "../data/vol-output/ml_base_predictions_v.csv")
cat("✅ Saved ../data/vol-output/ml_base_predictions_v.csv\n")

# === Save metrics ===
metrics_df <- map_dfr(results, as_tibble, .id = "Model")
write_csv(metrics_df, "../data/vol-output/ml_model_metrics_v.csv")
cat("✅ Saved ../data/vol-output/ml_model_metrics_v.csv\n")

r2_df <- metrics_df %>% select(Model, R2) %>% rename(Test_R2 = R2)
write_csv(r2_df, "../data/vol-output/ml_r2_v.csv")
cat("✅ Saved ../data/vol-output/ml_r2_v.csv\n")
