...
eval_metrics <- function(y_true, y_pred, vasa_preds_all = NULL, best_base_r2 = NULL) {
  ...
  # Tail RMSE
  tail_threshold <- quantile(y_true_clean, 0.95, na.rm = TRUE)
  tail_mask <- y_true_clean > tail_threshold
...
r2_base <- map_dbl(base_models, ~ {
  ...
  complete_cases <- complete.cases(y_test, test_pred)
  cor(y_test[complete_cases], test_pred[complete_cases])^2
})
...
write_csv(r2_df, '../data/vasa_r2_v.csv')
cat("✅ Saved ../data/vasa_r2_v.csv\n")

metrics_df <- bind_rows(
  as.data.frame(metrics_train) %>% mutate(Set = "Train"),
  as.data.frame(metrics_test) %>% mutate(Set = "Test")
) %>% select(Set, everything())
write_csv(metrics_df, '../data/vasa_metrics_v.csv')
cat("✅ Saved ../data/vasa_metrics_v.csv\n")
